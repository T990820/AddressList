...
void printAll(person* p,int N)
{
	...
	else
	{
		for(int i=0;i<N;i++)
		{
			cout<<"第"<<i+1<<"位联系人的姓名是："<<(p+i)->name<<endl; 
			cout<<"第"<<i+1<<"位联系人的性别是："<<(p+i)->sex<<endl;
			cout<<"第"<<i+1<<"位联系人的手机号是："<<(p+i)->cell<<endl;
			cout<<"第"<<i+1<<"位联系人的住址是："<<(p+i)->addr<<endl;
			cout<<"第"<<i+1<<"位联系人的年龄是："<<(p+i)->age<<endl;
		}
		cout<<endl;
	}
}
